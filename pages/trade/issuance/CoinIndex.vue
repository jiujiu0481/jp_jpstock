	...
	<view :class="isAnimat?'fade_in':'fade_out'" class="page_bg">
		...
		<view style="display: flex;align-items: center;justify-content: center;">
			...
			<view style="display: flex;align-items: center;justify-content: space-between;margin:  10px 10px 20px; ">
				<view  style="padding: 8px 30px; background-color:#d7060f;color: #FFFFFF;border-radius: 20px;font-size: 14px;margin-left: 20px;"  @click="$u.route({url:'/pages/trade/issuance/CoinRecord'});">申請記録</view>
				<view  style="padding: 8px 30px; background-color:#d7060f;color: #FFFFFF;border-radius: 20px;font-size: 14px;margin-right: 20px;" @click="$u.route({url:'/pages/trade/issuance/CoinRecord?curTab=1'});">成功の記録</view>
			</view>



			<view style="border-radius: 10px;margin: 0 20px;">
				<CoinTradeIssuanceList ref="list"></CoinTradeIssuanceList>
			</view>
...
<script>
	...
	import CoinTradeIssuanceList from './components/CoinTradeIssuanceList.vue';
	export default {
		components: {
			HeaderThird,
			CoinTradeIssuanceList,
		},
		data() {
			return {
				isAnimat: false, // 页面动画
