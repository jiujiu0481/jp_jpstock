				...
				<view class="common_block" style="border-radius: 16rpx;">
					<view style="display: flex;align-items: center;">
						...
						<view style="margin-left: auto;" @tap="handleCanel(item.id)">
							<view :style="setBtnStyle()">キャンセル</view>
						</view>
					</view>
	...
	export default {
		...
		methods: {
			...
			async handleCanel(val) {
				const result = await this.$http.get(`api/product/hy_cancel`, {
					id: val
				});
			...
			async getList() {
				...
				this.list = temp.length <= 0 ? [] : temp.map(item => {
					return {
						...
						directText: item.direct == 1 ? '購入' : '売却',
